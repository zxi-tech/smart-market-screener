...
class MarketDataDownloader:
    ...
    def fetch_ticker_data(self, ticker: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if df.empty:
                print(f"[!] Data kosong atau ticker tidak ditemukan: {ticker}")
                return None
            ...
            print(f"[x] Gagal memproses {ticker}. Pesan galat: {e}")
            return None

    def run(self):
        print(f"=== Memulai Data Pipeline: {len(self.tickers)} Ticker ===")
        for ticker in self.tickers:
            ...
            if data is not None:
                ...
                print(f"[v] Tersimpan: {file_path}")
        print("=== Proses Selesai ===")

if __name__ == "__main__":
    daftar_pantauan = ["BBCA.JK", "GOTO.JK", "AMMN.JK", "AAPL"]
    
    lokasi_simpan = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../data/raw"))
    
    pipeline = MarketDataDownloader(
        tickers=daftar_pantauan,
        start_date="2023-01-01",
        end_date="2026-03-01",
        output_dir=lokasi_simpan
    )
